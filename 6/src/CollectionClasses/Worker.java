...
public class Worker implements Serializable, Comparable<Worker> {
    ...
    @NumberValueControl.CompareNumbersValues(valueMin = 0, inclusivelyMin = false)
    @CsvBindByName
    transient private double salary; //Значение поля должно быть больше 0
...
//            i.check(id);
    ...
    public String toString() {
        return  "id: " + ((id == null) ? "null" : id) + "\n" +
                ...
                "дата создания: " + ((creationDate == null) ? "null" : (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(creationDate)) + "\n" +
                "зарплата: " + salary + "\n" +
