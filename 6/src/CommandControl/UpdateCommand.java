    ...
    static {
        sentCommand = new SentCommand() {
            ...
            public Object execute(HashSet<Worker> collection) {
                ...
                collection.removeIf(worker1 -> worker1.getId().equals(Integer.parseInt(arguments[0])));
                worker.setId(Integer.parseInt(arguments[0]));
    ...
    public void run() throws ServerException {
        if (argumentsTool.getArguments() != null && argumentsTool.getArguments().length >= 1) {
            try {
                ...
                boolean exists = (Boolean) clientServer.sendAndGet(new CheckIfWorkerIdExistsCommand(id));
                if (!exists) {
                    ...
                    clientServer.send(sentCommand.addWorker(workerRequiredTool.getInputWorker().askWorker()).addArguments(argumentsTool.getArguments()));
                }
        ...
        if (argumentsTool.getArguments() != null && argumentsTool.getArguments().length >= 1) {
            try {
                ...
                if (!exists) {
                    ...
                    clientServer.send(sentCommand.addWorker(workerRequiredTool.getScriptDataManager().getWorker()).addArguments(argumentsTool.getArguments()));
